...
- Lista los juegos disponibles como tarjetas (nombre + resumen + acceso). Sumar un juego nuevo es agregar una entrada al catálogo (con su clave en `game_results`) y su pantalla.
- Cada tarjeta muestra los números del usuario en ese juego, si los hay: si **el del día** ya está resuelto, la **racha** y el **mejor tiempo**.

## El del día, los tiempos y la racha

Los dos juegos comparten la misma mecánica de persistencia:

- **El puzzle del día:** cada juego abre en un tablero fijo por fecha, **igual para todos** — la fecha siembra un RNG determinístico (`resources/js/rng.js`, mulberry32) que reemplaza a `Math.random` en el generador, sin presupuesto de tiempo (cortar por reloj daría tableros distintos por dispositivo). La fecha la fija el servidor al servir la página, en la zona horaria de la casa (`config('amparo.zona_horaria')`, Argentina por defecto), así el día no cambia a las 21:00. Con el chip **Libre** se juegan tableros al azar, como siempre; **Tablero nuevo** existe solo ahí. Si el del día ya está resuelto, la página abre en Libre (se puede volver a jugar el del día por gusto).
- **Tiempos y racha:** al ganar, el tablero avisa al servidor (`recordWin`, trait `RecordsGameResults`) y se guarda una fila en `game_results` (`user_id`, juego, si fue diaria, fecha, segundos). Del puzzle del día cuenta **solo la primera victoria de cada fecha** (repetirlo no pisa el tiempo); las libres se acumulan. El **mejor tiempo** junta diarias y libres; la **racha** son los días consecutivos con el diario resuelto, mirando hacia atrás desde hoy — si hoy todavía no jugaste, la racha de ayer sigue viva. Una victoria "diaria" con fecha vieja (una pestaña abierta de otro día) vale como libre: el tablero resuelto no era el del día.
- **El aviso al servidor es lo único que sale del navegador**: si falla (sin conexión, sesión vencida), la partida no se pierde — solo queda sin anotar. El tiempo lo informa el cliente; alcanza para uso personal, pero no serviría de base para un ranking entre usuarios (eso exigiría validar la partida en el servidor).
- La partida **a medias no se guarda**: salir y volver arranca de cero (retomarla está en el backlog, ver [TODO.md](../TODO.md)).
- Como siempre, cada usuario ve solo sus números (`auth()->user()->gameResults()`).
...
- **Todo pasa en el navegador.** La partida se genera y se juega en el cliente (Alpine + el generador de `resources/js/queens.js`): abrir la página o pedir un tablero nuevo **no hace ninguna llamada al servidor**, así responde al toque al instante. El componente Livewire entrega el marco de la página con los números del usuario; el servidor solo entra al ganar, para anotar el resultado (ver "El del día, los tiempos y la racha" arriba).
...
- **Los mejores tiempos y la racha del día se guardan** (ver la sección compartida arriba); la partida a medias no — salir y volver arranca de cero.
...
- **Todo pasa en el navegador**, igual que Queens: abrir la página o pedir un tablero nuevo no llama al servidor; el aviso de victoria es la única llamada.
...
- **Los mejores tiempos y la racha del día se guardan**, como en Queens (ver la sección compartida arriba); la partida a medias no.
