...
- **Archivos subidos (adjuntos de Salud y los que vengan) van al disk por defecto de filesystems, nunca al disco del contenedor a mano.** En Cloud eso exige un **bucket de object storage adjuntado al environment** (al adjuntarlo, Cloud inyecta solo `FILESYSTEM_DISK=s3` y las `AWS_*`; el paquete `league/flysystem-aws-s3-v3` ya está en composer). Se sirven por rutas autenticadas que **streamean el archivo con disposición de descarga** — nunca con visibilidad pública ni URLs permanentes, y tampoco redirigiendo a URLs firmadas externas: en la PWA instalada esa navegación fuera del scope abre otro contexto y "volver" saca al usuario de la app. Ojo: las subidas temporales de Livewire van al mismo disk, y su driver S3 **no soporta inputs con `multiple`** — los archivos se eligen siempre de a uno (no se reproduce en local, revienta solo en producción).
